...
async function run() {
  try {
    ...
    console.log(dryRun ? "Running in dry-run mode. No licenses will be issued." : "Processing reconciliation jobs.");
    ...
    for (const contract of contracts) {
      ...
      if (!contract.device || !contract.device.id) {
...
Contract: ${contract.contractNumber} (${contract.id})
  Dealer: ${contract.dealerId}
  Device: ${contract.device.id}
  Customer: ${contract.customerId}
  Latest successful payment: ${payment.id} (${payment.providerReference})
  Contract status: ${contract.status}
  License type expected: ${contract.status === ContractStatus.completed ? 'permanent' : 'temporary'}
`);
