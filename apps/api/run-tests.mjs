...
async function test1_SignupWithBankAccount() {
  ...
  try {
    ...
    if (response.status === 201 && data.data) {
      ...
      console.log(`   Paystack Subaccount Code: ${data.data.dealer.paystackSubaccountCode || 'NOT SET (Paystack may be disabled)'}`);
...
async function test3_LoginWithDealerCredentials(dealerInfo) {
  if (!dealerInfo) {
...
async function runAllTests() {
  ...
  try {
    ...
    const test2Result = await test2_SignupWithMissingSettlementInfo();
    ...
    console.log('✅ Test 2: Rejected missing settlement info - ' + (test2Result ? 'PASSED' : 'FAILED'));
    console.log('✅ Test 3: Dealer login - ' + (token ? 'PASSED' : 'FAILED'));

    if (dealerInfo && token && test2Result) {
      console.log('\n🎉 ALL TESTS PASSED!');
      console.log('\nDealer Creation & Login Flow Summary:');
